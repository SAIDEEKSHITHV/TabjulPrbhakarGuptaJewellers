...
export default function AddProductPage() {
  ...
  const processUploads = async (fileList: File[]) => {
    ...
    setError(null);
    try {
      const folderPath = `tpg-jewellers/products/temp-add-${Date.now()}`;
      ...
      setError(err instanceof Error ? err.message : 'Image upload failed. Check file types and sizes.');
    } finally {
    ...
    try {
      ...
      if (images.length > 0) {
        ...
        const { error: imgErr } = await supabase
          .from('product_images')
          .insert(finalImagesPayload);

        if (imgErr) {
          throw new Error(`Product saved successfully, but saving image references failed: ${imgErr.message}. (Created Product ID: ${productId})`);
        }
      }
      ...
      console.error('Error inserting product catalog:', err);
      setError(err instanceof Error ? err.message : 'An error occurred during catalogue insert.');
    } finally {
      setIsSaving(false);
    }
  };
